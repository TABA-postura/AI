...
## 🧩 판별 로직: 규칙 기반 + 모델 기반(2단계)

본 시스템은 실시간성(낮은 지연)과 판별 신뢰도(오탐/미탐 최소화)를 동시에 확보하기 위해, 1차 규칙 기반 판별과 2차 모델 기반 검증을 결합한 하이브리드 구조로 동작합니다.

### 설계
<details>
  <summary> 1차(규칙 기반): MediaPipe Pose/Hands 랜드마크로 메트릭을 계산해, 빠르고 해석 가능한 자세 판정 수행 </summary>
...
<details>
  <summary> 2차(모델 기반): 규칙 기반 결과가 애매한 경계 상황일 때, MobileNetV2 분류 모델로 추가 검증하여 오탐 감소 및 신뢰도 보강 </summary>
...
<details>
  <summary> 결합 방식(Fusion/Gating): 아래 원칙을 기반으로 결합하여 실시간성, 신뢰도 확보 </summary>
...
<img width="800" alt="image" src="https://github.com/user-attachments/assets/e9227627-e232-4fc2-aaf1-2c542bccc843" />
